...
describe("User authentification", () => {
  ...
  beforeEach(() => {
    ...
    cy.fixture("fakeRequestApi.json").then((data) => (_data = data));
  ...
  context("Login input validation", () => {
    ...
    it("should show error when email is invalid ", () => {
      cy.get("[data-cy=email]").type("Sowell");
  ...
  context("I am a valid user with role", () => {
    it("should fill login form with user credentials and redirect to homepage on submit", () => {
      ...
      cy.get("[data-cy=email]")
        ...
        .should("have.value", _credentials.validUser.email);
      cy.get("[data-cy=password]")
        .type("SW-@sowell")
        .should("have.value", _credentials.validUser.password);
      cy.get("[data-cy=submit-login]")
        ...
        .then(() => {
          ...
          cy.intercept({
            ...
            url: "/users/sign_in",
            body: {
              email: _credentials.validUser.email,
              password: _credentials.validUser.password,
            },
          }).as("loginPost");
    ...
    it("should fill valid mail but wrong password and get forbidden notification error", () => {
      ...
      cy.get("[data-cy=password]")
        .type("Sw-@sowell")
        .should("have.value", _credentials.validUser.passwordWrong);
      ...
      cy.get("[data-cy=submit-login]")
        ...
        .then(() => {
          cy.intercept("POST", "/users/sign_in", {
            body: {
              email: _credentials.validUser.email,
              password: _credentials.validUser.passwordWrong,
            },
          })
            .then((res) => {
  ...
  context("I am not a valid user", () => {
    it("should fill login form with user credentials and get error on submit", () => {
      ...
      cy.get("[data-cy=email]")
        ...
        .should("have.value", _credentials.invalidUser.email);
      cy.get("[data-cy=password]")
        .type("123456")
        .should("have.value", _credentials.invalidUser.password);
      // Locate and submit the form
      cy.get("[data-cy=submit-login]")
        .click()
        .then(() => {
          cy.intercept("POST", "/users/sign_in", {
            ...
            .then((res) => {
              ...
              //ToDO: Verify token and role
              expert(res.data.token).to.eq(_data.token);
              //Decode token JWT  imported in command.js
            })
            .as("loginPost");
        });
